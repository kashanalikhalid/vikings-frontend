...
import {deleteMember,femaleMembers} from "../actions/memberActions.js";


const FemaleMember=({history, location})=>{

    const dispatch= useDispatch()
    const memberListData =useSelector(state=>state.femaleMembers)
    ...
    useEffect(()=>{
        if(location.search){
            dispatch(femaleMembers(location.pathname+location.search))
        }
        else{
            dispatch(femaleMembers(location.pathname))
        }
    ...
    const handleSearch= (value,event)=>{
        ...
        setSearch(value)
        if(value!=='')
            history.push(`/admin/females?search=${value}`)
        else{
            history.push(`/admin/females`)
        }
    }
    ...
    const pageChangeHandler=(page)=>{
        ...
        if(search==='')
        {
            history.push(`/admin/females?page=${page}`)
        }
        else{
            history.push(`/admin/females?search=${search}&page=${page}`)
        }
    }
